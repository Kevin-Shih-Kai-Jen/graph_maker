...
def read_data(inner_data: str, sheet_name: str):
    ...
    match filetype:
        case ".csv":
            data = pd.read_csv(inner_data, dtype=str)
...
transposed_data = data.transpose().reset_index(drop=True)
...
final_data = transposed_data_cleaned.to_csv(output_file_path, index=False, header=True)
