...
例子：
/Users/coolguy/Desktop/經濟指標/程式/UNRATE.csv, UNRATE,  observation_date, UNRATE, 794 
/Users/coolguy/Desktop/經濟指標/程式/FEDFUNDS.xlsx, Monthly, observation_date, FEDFUNDS, 2
"""
...
def parameter_input() -> list[list[str]]:
    ...
    while 1:
        data = input(
            "請輸入以下格式 path_1, sheet_name（沒有的話請輸入0）, 日期名稱, 主要數據名稱, 第幾行: "
        )
...
def string_to_array(data: list[list[str]]) -> list[list[str]]:
    ...
    array_data = combine_five_data(array_data)

    return array_data


def combine_five_data(inner_data: list[str]) -> list[list[str]]:
    combined_data = []

    for i in range(len(inner_data) // 5):
...
def read_data(imported_data: list[list[str]]) -> list[pd.DataFrame]:
    ...
    for i in range(len(imported_data)):
        ...
        match file_type:
            case ".csv":
                path_data.append(pd.read_csv(imported_data[i][0]))

            case ".xlsx" | ".xls":
                path_data.append(
                    pd.read_excel(imported_data[i][0], sheet_name=sheets[i])
                )
...
def parse_data(
    ...
    column_parameters: list[list[pd.DataFrame]],
    ...
    for i in range(len(column_parameters)):
        line_length = len(pd_data[i][str(column_parameters[i][0])])
        actual_data = [
            pd_data[i]
...
def determine_whether_the_timeline_is_reverse_or_not(
    ...
    first_line = pd.to_datetime(inner_date.iloc[0])
    second_line = pd.to_datetime(inner_date.iloc[1])
...
def the_same_format_date(date_data: list[pd.DataFrame]) -> list[pd.to_datetime]:
    same_format_date_data = []

    try:
        for item in date_data:
            same_format_date = pd.to_datetime(item, format="mixed", errors="raise")
            same_format_date_data.append(same_format_date)

        return same_format_date_data
...
if __name__ == "__main__":
    ...
    same_format_date = the_same_format_date(main_date_data)
    draw_graph = DrawGraph(same_format_date, main_other_than_date_data)
